...
fn lex_rpalen(input: &[u8], start: usize) -> Result<(Token, usize), LexError> {
    consume_byte(input, start, b')').map(|(_, end)| (Token::rpalen(Loc(start, end)), end))
}
...
fn lex(input: &str) -> Result<Vec<Token>, LexError> {
    ...
    while pos < input.len() {
        match input[pos] {
            ...
            b')' => lex_a_token!(lex_rpalen(input, pos)),
            b' ' | b'\n' | b'\t' => {
